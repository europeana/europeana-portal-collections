...
module I18nHelper
  ...
  def date_eras_gregorian(date)
    ...
    m = date.match(/^\d+$/)
    return suffix_gregorian_era_date(m[0], '+') if m && strip_leading_zeroes(m[0]).length < 4
    ...
    if m
      year = m[2]
      return suffix_gregorian_era_date(year, m[1]) unless year.include?('-') && !year.match(/^\d{1,4}\-\d{1,2}\-\d{1,2}$/)
    end
    ...
    ['c. AD', 'a. de C.', 'c.', 'circa'].each do |abbrev|
      ...
      return suffix_gregorian_era_date(m[1], '+') if m
    end
