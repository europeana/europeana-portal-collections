...
RSpec.describe 'entities/show.html.mustache', :common_view_components do
  ...
  it 'should have a title "Entity Name - Europeana Collections"' do
    render
    expect(rendered).to have_title('Entity Name - ' + t('site.name'))
  end
  ...
  it 'should have a title' do
    render
    expect(rendered).to have_selector("meta[property=\"og:title\"][content=\"#{entity.pref_label} - #{t('site.name')}\"]", visible: false)
  end
  ...
  describe '#entity_description_title' do
    context 'when entity type is agent' do
      it 'should use Biography as a label for the description' do
        render
        expect(rendered).to have_content('Biography')
      end
    end
  ...
  describe 'noindex header tag' do
    context 'when the entity has no records relating to itself' do
      ...
      it 'should be present' do
        render
        expect(rendered).to have_selector('meta[property="robots"][content="noindex"]', visible: false)
      end
    end
    ...
    context 'when the entity has no records relating to itself' do
      it 'should not be present' do
        render
        expect(rendered).to_not have_selector('meta[property="robots"][content="noindex"]', visible: false)
      end
    end
  end

  describe 'JS vars' do
    it 'should include schema.org JSON-LD URL' do
      render
      expect(rendered).to have_selector('script', text: %(var entitySchemaOrgUrl = "#{entity.schema_org_json_ld_url}";), visible: false)
    end
  end
end
